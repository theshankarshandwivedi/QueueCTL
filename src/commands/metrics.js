...
function serve(port = 8080) {
  const server = http.createServer((req, res) => {
    ...
    if (req.url === "/" || req.url.startsWith("/?")) {
      ...
      res.write(
        `<p>Total: ${stats.total} | Pending: ${stats.pending} | Completed: ${stats.completed} | Dead: ${stats.dead}</p>`
      );
      res.write("<h2>Jobs</h2><ul>");
      jobs.slice(0, 200).forEach((j) => {
        res.write(
          `<li>${j.id} - ${j.command} - ${j.state} - attempts:${j.attempts}</li>`
        );
      });
      res.write("</ul>");
      res.write("</body></html>");
  ...
  server.listen(port, () => {
    console.log(
      chalk.green(`Metrics server listening on http://localhost:${port}`)
    );
  });
